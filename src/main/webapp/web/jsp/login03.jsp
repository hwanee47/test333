 ...
 * 휴대폰 본인 인증 페이지 열기
 */
function openPccFormPop() {
	
	// 팝업 Option값
	var option = {
		url          : "web/common/popup/getPopPccForm.fo",
		svcId        : THIS_SVC_ID,
		popName      : "getPopPccForm",
		width        : 450,
		height       : 550
	};
...
function fn_getTempPWByEmail(){
	...
	if(id == "") {
		...
		alert('<%= MessageUtil.getString("msg.find.0011", "담당자 ID를 입력하여 주십시오.") %>');
		$("#userId").focus();
		return;
	}
...
<div class="container">
	<!-- content -->
	<div class="content">
		<h2 class="s_tit"><%= MessageUtil.getString("msg.find.0012", "비밀번호 재발급") %></h2>

		<div class="sub_inner_ty01">
			<div class="find_box01">
				<div class="top_tx_bx">
					<dl>
						<dt><%= MessageUtil.getString("msg.find.0013", "임시 비밀번호 발급받기") %></dt>
						<dd><%= MessageUtil.getString("msg.find.0014", "회원정보에 등록 된 이메일 주소를 입력하시면 이메일로 임시비밀번호를 발송하여 드립니다.") %></dd>
					</dl>
				</div>
				<div class="inp_wrap">
					<div class="inp_bx">
						<input type="text" class="inp_ty01" id="userId"/>
						<p class="tx01"><%= MessageUtil.getString("msg.find.0015", "가입 시 등록하신 아이디를 입력하세요") %></p>
					</div>
					<div class="inp_bx">
				...
				<div class="btn_area"><a href="#" class="btn_ty01" onClick="fn_getTempPWByEmail()"><%= MessageUtil.getString("msg.find.0004", "간편 찾기") %></a></div>
			</div>
