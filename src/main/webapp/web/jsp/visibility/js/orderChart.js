...
'		<backgroundElements>'+
'			<CanvasElement>'+
'				<Label	text="$$RATE$$%" height="24" horizontalCenter="0" verticalCenter="0" fontSize="19" color="#FFFFFF" backgroundAlpha="0"/>'+
'			</CanvasElement>'+
'		</backgroundElements>'+
'	</Pie2DChart>'+
'</rMateChart>'
...
function initChart() {
	...
	var	arrBaseCorp		= ORD.getArrBaseCorp();
	var	item			= null;
	...
	for(var i=0; i<arrBaseCorp.length; i++) {
		...
		// 차트 Data 설정
		arrData		= [	{"STATUS":lang("0052", "배차")	,"FILL":"#2e9dff"	,"CNT":item.VHCL_CNT},
		          		{"STATUS":lang("0074", "미배차")	,"FILL":"#2f4b68"	,"CNT":item.NA_CNT}		];
		...
		// 차트 Layout 설정 (가동률 적용)
		rate	= (item.TOT_CNT == 0)? 0 : Math.round(item.VHCL_CNT/item.TOT_CNT*100);
		CHART_LAYOUT_MAP.set(chartId, CHART_LAYOUT_BASE.replace("$$FILLS$$", fills).replace("$$RATE$$", rate	));
