...
pForm.gfnRemoveSpecialChar = function(strValue)
{
   ...
   if(this.gfnIsNull(strValue)) return strValue;
 ...
 * Return       : true / false
 *******************************************************************************/
pForm.gfnIsEmpty = function (Val)
{
	return this.gfnIsNull(Val);
}


/*******************************************************************************
 * Function Name: gfnStrDsToObjDs
 * Description  : 문자열 dataset을 object dataset으로 변환
 * Arguments    : saveXML형식의 문자열(dataset)
 * Return       : object Dataset
 *******************************************************************************/
pForm.gfnStrDsToObjDs = function (strDs)
{
	var objDs = new Dataset;
	if(!this.gfnIsNull(strDs)) {
		objDs.loadXML(strDs);
	}
 ...
 * Return       : String
 *******************************************************************************/
pForm.gfnToString = function (Val)
{
 ...
 * return       : 대체된 문자열
 **********************************************************************************/
pForm.gfnReplace = function (Val, strOld, strNew)
{
   //trace("=== gfnReplace ===");
	Val = this.gfnToString(Val);
 ...
 ********************************************************************************/
pForm.gfnIndexOf = function (Val, strOld, index)
{
	if (this.gfnIsNull(index)) 
	{
		index = 0;
	}
	return this.gfnToString(Val).indexOf(strOld, index);
}

/******************************************************************************
 * Function Name: gfnNumFormat(dNumber);
 ...
 * gfnNumFormat(12345.66)
 ******************************************************************************/
pForm.gfnNumFormat = function (dNumber, nDetail)
{
	var strVal = "" + dNumber;
	var rtnStr;
	var arrStr;

	if (this.gfnIsNull(nDetail) == false) 
	{
		strVal = TOBE.toNumber(dNumber).round(nDetail);
	}
	return this.gfnSetComma(strVal);
}

/******************************************************************************
 * Function Name: gfnGetFormat
 * Description  : 문자 포맷 형식 변환 ( 해당 문자는 포맷에서 @ 사용)
 * Arguments    : Val: 문자열,
 *                strMask: 문자열 포맷, (@:문자, 포맷스트링("-", ",", ".")등
 * return       : 포맷된 문자
 ******************************************************************************/
pForm.gfnGetFormat = function (Val, strMask)
{
	var strRetVal = "";
	var sUnit;
 ...
 ******************************************************************************/
pForm.gfnSplit2 = function (Val, strDelimiter1, strDelimiter2)
{
...
/******************************************************************************
 * Function Name: gfnFindArray
 * Description  : 1차원 배열에서 입력된 문자열이 있는 index를 반환
 * argument     : ArrObj        : 1차원 배열
 strKey        : 검색할 문자열
 * return       : 찾은 배열 index
 ******************************************************************************/
pForm.gfnFindArray = function (ArrObj, strKey)
{
 ...
 * return       : 찾은 배열 index
 ******************************************************************************/
pForm.gfnFindArray2 = function (Arr2Obj, strKey, index)
{
...
/***********************************************************************************************
 * @function	: gfnGetCalculDate
 * @description	: 날자/시간 계산 return 함수
 * @param 		: - type : 시간계산 구분 : hour - 시간단위 계산
										   : second - 초단위 계산
				  - time : 계산시간
				  - fromDate : 지정일시 : 지정일로부터 시간을 계산할 경우 입력
 * @return 		: 계산된 일자
/***********************************************************************************************/ 
pForm.gfnGetCalculDate = function(type, time, fromDate) 
{
	var s;
	var d = new Date();	

	if(!this.gfnIsNull(fromDate)) {
		var y = fromDate.substr(0, 4);
		var m = fromDate.substr(4, 2);
		var d = fromDate.substr(6, 2);
		var hh = fromDate.substr(8, 2);
		var mi = fromDate.substr(10, 2);
		var ss = fromDate.substr(12, 2);
		d = new Date(y, m-1, d, hh, mi, ss);
	}
	...
	s = d.getFullYear()
	  ...
	  + (d.getDate() + "").padLeft(2, '0')
	  + (d.getHours()+ "").padLeft(2, "0")
	  + (d.getMinutes() < 60 && d.getMinutes() > 29? "30" : "00")
	  + "00";
	return (s);
};

/***********************************************************************************************
 * @function	: gfnCheckJumin
 * @description	: 주민등록번호 적함성 여부 체크 함수
 * @param 		: val1-주민번호앞6자리 val2-주민번호뒤7자리

 * @return 		: 1(적합), -1(부적합)
/***********************************************************************************************/ 
pForm.gfnCheckJumin = function( val1, val2 ) {
	var tmp1, tmp2, tmp3;
	...
	if( this.gfnDoCheckProResnoLength( val1 ) == -1 ) return -1;
	if( this.gfnDoCheckPreResnoLength( val2 ) == -1 ) return -1;
	...
	var tot = nexacro.toNumber(t1)  * 2 + nexacro.toNumber(t2)  * 3 + nexacro.toNumber(t3)  * 4 + nexacro.toNumber(t4)  * 5 + nexacro.toNumber(t5)  * 6 + nexacro.toNumber(t6)  * 7;
	tot    += nexacro.toNumber(t11) * 8 + nexacro.toNumber(t12) * 9 + nexacro.toNumber(t13) * 2 + nexacro.toNumber(t14) * 3 + nexacro.toNumber(t15) * 4 + nexacro.toNumber(t16) * 5 ;
...
// 주민번호 형식 check (앞 6자리)
pForm.gfnDoCheckProResnoLength = function( szProResno ) {
...
// 주민번호 형식 check (뒤 7자리)
pForm.gfnDoCheckPreResnoLength = function( szPreResno ) {
...
/***********************************************************************************************
 * @function	: gfnCalculRateClsAmt
 * @description	: 요율구분, 요청수송량 에따른 금액 계산
 * @return 		: amt - 요율구분, 요청수송량 당 계산금액
/***********************************************************************************************/ 
pForm.gfnCalculRateClsAmt = function(rateClsCd, rateAmt, reqQty) {
	var amt = 0;	
	rateAmt = nexacro.toNumber(rateAmt, 0);
	reqQty 	= nexacro.toNumber(reqQty, 1);
	...
	return nexacro.round(amt ,2);
}

/***********************************************************************************************
 * @function	: gfnConversionTonCode
 * @description	: 톤급명을 톤급코드로 변경한다
 * @param 		: tonNm : 변환할 톤급명
 * @return 		: N/A
/***********************************************************************************************/
pForm.gfnConversionTonCode = function(tonNm) {
	var tonNmArray = tonNm.toString().split('.');
	var a = tonNmArray[0];
	a = a.length > 3 ? a.substring(a.length-3, a.length) : a.length >= 3 ? a : new Array(3 - a.length +1).join('0') + a;

	var b = 0;
	if(tonNmArray.length > 1) {
		b = tonNmArray[1];
		b = b.length == 0 ? 0 : b.length > 1 ? b.substr(0, 1) : b;
	}
	return a + b;
}
