...
if (!nexacro._ShadowObjectNew) {
	...
	nexacro.ShadowObjectNew = function (val) {
		if(val instanceof nexacro._ShadowObjectNew) return val;
...
if (!nexacro._ChartBase)
{
    ...
    _pChartBase.userControlDraw = function(item)
	{
		...
		if(item)
		{
			item._updateStyles(ctx);
			item._setGlobalTransforms();
			item._render(ctx);
		}
		*/
	...
	{
		if(!this._seriesGroup || !id || !type) return;
   ...
   _pChartBase.setCategoryaxis = function (val)
    {
		...
		//if(chartName != "BasicChart" && chartName != "RoseChart" && chartName != "FloatChart") return;
    ...
    _pChartBase.getSeriesPointPos = function(seriesIdx, dataIdx)
	{
		...
		if(items && items[dataIdx])
		{
			...
			if(item._slice && item.pathData)
			{
				...
				if(this.valueaxes) maxRadius -= (this.valueaxes[0] ? this.valueaxes[0]._allaxisgapsize : 0);
				returnData = { 	"startangle" : item.startAngle, "endangle" : item.endAngle, 
								"rstartangle" : item.rStartAngle, "rendangle" : item.rEndAngle, 
								"angle" : item.angle, "bound" : item._slice._bounds,
								"centerx" : this._centerLeft, "centery" : this._centerTop ,
								"posinfo" : item._posinfo,
								"radius" : maxRadius };
			}
			else
			{
				returnData = { "x" : item.x, "y" : item.y, "bound" : item._bounds, "radius" : item.radius, "matrix" : item._matrix};
			}
		}
		return returnData;
	};
    ...
    _pChartBase.getAxisPointPos = function(type, value, axisindex, tickindex)
	{
		...
		if(checkXI >= 0)
		{
			...
			return { "x" : elem.x1 + axis._tickstartgap, "y" : elem.y2, "point" : elem._point };
		}
		return null;
	};		
            ...
            for (i = 0; i < len ; i++)
            {
                ...
                if (targets[i]["_series"])
                {
                    if (targets[i]["_legend"])
                    {
						...
						else	// if (this._type_name == "BasicChart" || this._type_name == "BubbleChart" || this._type_name == "RadarChart" || this._type_name == "RoseChart" || this._type_name == "FloatChart")
						{
							index = nexacro._ChartGraphicsLibArray.indexOfProp(this.seriesset, "id", targets[i]["_series"].id);
						}
                    }
                    else
                    {
						...
						else	// if (this._type_name == "BasicChart" || this._type_name == "BubbleChart" || this._type_name == "RadarChart" || this._type_name == "RoseChart" || this._type_name == "RoseChart" || this._type_name == "FloatChart")
						{
							if (overitemid.search("SeriesItemText_") >= 0)
							{
        ...
        if(e)
		{
			switch(e.reason)
			{
				...
				case nexacro.NormalDataset.REASON_RESET:
				{
					...
					if(this._changedData)
					{
						this.on_apply_contents();         // 2019.9.10

						if(this.enableanimation)         //  위에 있던거 아래로 이동						
						{						
							this._loadanimation = true;						
						    //이후 처리에서 draw여부 판단함						
						}

						// event가 중복(rowposchange)해서 타면서 발생하는 문제임						
						// 원래 rowposition이 0으로 이동된건가?? 햇갈림.						
						obj.set_enableevent(false);

						if(obj.rowcount > 1) obj.set_rowposition(0);
                    ...
                    if (seriesItem) // onmouseenter
                    {
                        ...
                        if (seriesItemType)
                        {
                            ...
                            else if (searchpoint || searchline ||  searcharea || searchgauge || _firstseriesitem/*seriesItemType == "Ellipse" || seriesItemType == "Paths" || seriesItemType == "Path"*/)
                            {
								...
								else if (chartName == "RoseChart")	// hsji
								{
                              ...
                              /*  if (!nexacro._ChartGraphicsLib.isEmpty(index) && !nexacro._ChartGraphicsLib.isEmpty(value))
                                {
                                    evt.index = index;
                                    evt.value = value;
                                }*/
    ...
    _pChartBase._createSubControl = function (contents)
    {
        ...
        nexacro._ChartGraphicsLibArray.forEach(this._createdOrder,
            function (name, index)
            {
                if (!contents.hasOwnProperty(name))
                {
                ...
                {
                    nexacro._ChartGraphicsLibArray.forEach(value,
                        function (o, idx)
                        {
                            if (name == "seriesset")
                            {
    ...
    _pChartBase._arrangeTitle = function ()
    {
        ...
        if (title && title.visible)
        {
            var boardRect = this._boardRect,
                ...
                //titlerotate = title._titlerotate,
            var xSize = 0;
  ...
  */        
    ...
    _pChartBase._drawStackTotal = function ()
    {
		...
		for (listone in stacklist)
		{
			...
			{
				listoneTmp = listone;
				if(!listoneTmp || listoneTmp == "") listoneTmp = "_default_";
		...
		// item text : group인 경우 bar가 붙어 text가 짤림.
		...
		if(grpTextList && grpTextList.length > 0)
		{
			...
			{
				if(!grpTextList[z] || grpTextList[z].length == 0) continue;
				seriesItem = grpTextList[z];
				for(var zs=0;zs<seriesItem.length;zs++)
				{
					data = seriesItem[zs];
					seriesset[z]._drawBarItemText(data.left, data.right, data.bottom, 
											data.top, data.width, data.height, data.rect);	
				}
			}
		}
		if(grpTextList)
...
_pChartBase._setChangeInBoardAreaPos = function (item)
{
	if(item && item._type_name == "GraphicsText")
	{
		...
		var labelBoundRect = item.getGlobalBoundRect(),
...
if (!nexacro._AxisChartBase)
{
    ...
    _pAxisChartBase._setDatapoint = function ()
    {
        var seriesset = this.seriesset,
           s, 
		   //categoryaxis, valueaxis, ps, sdata,
           pointshape,
           xaxis, yaxis;
...
if (!nexacro.ChartAxisControl)
{
	...
	//_pChartAxisControl.smalltickcount = 0;	// hsji	- 추후 이동 / gauge에만 적용.
    ...
    _pChartAxisControl._tickintervalcategory = 0;	// 8.28
...
*/
    ...
    _pChartAxisControl._setTicks = function (istotal)
    {
		...
		if(subTick > 1 && tickLength > 1)
		{
			var v0 = tickGen[0];
			var v1 = tickGen[1];
			if (typeof v0 == "object") v0 = tickGen[0];
			if (typeof v1 == "object") v1 = tickGen[1];
            ...
            moved: function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY)
            {
                ...
                else if (dragged)
                {
                    var start = 0, end = 0,
						...
						boundW = 0,
                        boundH = 0,
						//datamin = pThis._axis._tickmin || pThis._axis._datamin,
						datamax = pThis._axis._tickmax || pThis._axis._datamax,
						//datastart = pThis.p2cScroll(datamin),
                        startpadding,
                        endpadding,
...
*/
    ...
    _pChartSelection._createSelection = function ()
    {
        if (!this._selectionRect)
        {
            //this._selectionRect = new nexacro.ChartGraphicsRect(0, 0, 0, 0);
			...
			ended: function (button, alt_key, ctrl_key, shift_key, screenX, screenY, canvasX, canvasY, clientX, clientY)
			{
				...
				if(ret == true)
				{
					if(pThis._type.indexOf("x") != -1)
					{
						...
						{
							//pThis._xstartdata = pThis._extractRange(pThis._xaxis,Math.min(pThis._info.first.x , pThis._info.second.x) - boardRect.left - tickgap);
							//pThis._xenddata = pThis._extractRange(pThis._xaxis,Math.max(pThis._info.first.x , pThis._info.second.x) - boardRect.left - tickgap);
					...
					if(pThis._type.indexOf("y") != -1)
					{
						...
						if (pThis._checkMinsizeSelection())
						{
							//pThis._yenddata = pThis._extractRange(pThis._yaxis,Math.min(pThis._info.first.y , pThis._info.second.y) - boardRect.top - tickgap);
							//pThis._ystartdata = pThis._extractRange(pThis._yaxis,Math.max(pThis._info.first.y , pThis._info.second.y) - boardRect.top - tickgap);
    ...
    _pChartSelection._drawSelection = function (gridRectBound,all)
    {
        ...
        if (info.show && this._checkMinsizeSelection())
        {
            if (selectionRect)
            {
				...
				//if(type == "x")h = gridRectBound.height -1;
                //if(type == "y")w = gridRectBound.width -1;
...
if (!nexacro._SeriesBase)
{
  ...
  // pointshape 관련 객체 정의
	...
	/*
	_pSeriesBase._shadeColor = function (color, percent) 
	{
		if(color == "transparent" || color == "") return color;
		if(nexacro._xreNamedColorList[color]) color = nexacro._xreNamedColorList[color];
	...
	*/
	_pSeriesBase._shadeColor = function (hueIn, pcIn) {
		if(hueIn == "transparent" || hueIn == "") return hueIn;
		if(nexacro._xreNamedColorList[hueIn]) hueIn = nexacro._xreNamedColorList[hueIn];
		if(hueIn.substring(0,1) != "#") return hueIn;
		if(hueIn.length < 7) return hueIn;
		...
		var pc    = parseInt(pcIn, 10);    //shade positive, tint negative
		var max   = 0;
		var dif   = 0;
		...
		{    //tint: make lighter
			...
			return 	"#" + ((truncate(red + dif)).toString(16)).padLeft(2, '0')  + 
					((truncate(green + dif)).toString(16)).padLeft(2, '0') + 
					((truncate(blue + dif)).toString(16)).padLeft(2, '0');
    ...
    _pSeriesBase._setData = function ()
    {
        ...
        if (chart)
        {
			...
			if(charttype == "PyramidChart")
			{
				...
				{
					if(this._chart.graphtype == "pyramid") data.sort(sortDFunction);
					else data.sort(sortAFunction);
				}
				else
				{
					if(this._chart.graphtype == "pyramid") data.sort(sortAFunction);
					else data.sort(sortDFunction);
				}
			}
            this._data = data;
    ...
    _pChartBoardControl._arrange = function (left, top, width, height)
    {
        ...
        if (gridCenterText)
        {
            ...
            if (text)
            {
                ...
                ////debug
                //var circle1 = new nexacro.ChartGraphicsEllipse(centerX, centerY, 8, 8);
...
if (!nexacro.ChartTitleControl)
{
    ...
    _pChartTitleControl._arrange = function (left, top)
    {
        ...
        if (this._titleText && titleTextWidth > 0)
        {
            ...
            //// debug
            //var circle2 = new nexacro.ChartGraphicsRect(textLeft, textTop, 5, 5);
        ...
        if (this._subTitleText && subTextWidth > 0)
        {
            ...
            //// debug
            //var circle1 = new nexacro.ChartGraphicsRect(subTextLeft, subTextTop, 5, 5);
...
if (!nexacro.ChartLegendControl)
{
    ...
    _pChartLegendControl._createItems = function ()
    {
        ...
        for (var i = 0; i < dataLen; i++)
        {
            // if (nexacro._ChartGraphicsLib.isEmpty(legendData[i].data))
            // {
            // break;
            // }
...
*/
    ...
    _pChartTooltipControl._showTooltip = function (obj, canvasX, canvasY)
    {
		...
		if(!this.background)
		{
			if(obj.fillstyle) this.on_apply_background(obj.fillstyle);
			else this.on_apply_background("#4b4b4b");
		}
		else
		{
			this.on_apply_background(this._background);
        ...
        var charttype = this.parent._type_name,
			text;
		/*
        if (charttype == "BasicChart" || charttype == "BubbleChart" || charttype == "RadarChart" || charttype == "GaugeChart" || charttype == "RoseChart" || charttype == "FloatChart")
        {
       ...
       // tooltipHeight = txtHeight + (padding * 2) + borderwidth;
		...
		if(((tooltipWidth / 2) < nRoundPix) || ((tooltipHeight / 2) < nRoundPix)) nRoundPix = 0;
		
		var fst_left = left;
		var fst_top = top;
        //tooltipPath.moveTo(left, top);
		if(nRoundPix > 0) tooltipPath.moveTo(left+nRoundPix, top);
		else tooltipPath.moveTo(left, top);

        // right top
        left += tooltipWidth;
        //tooltipPath.lineTo(left, top);
		if(nRoundPix > 0) tooltipPath.lineTo(left-nRoundPix, top);
		else tooltipPath.lineTo(left, top);	
		...
		if(nRoundPix > 0) 
		{
			tooltipPath.quadraticCurveTo(	new nexacro.Point(sv_left-nRoundPix, sv_top),
											new nexacro.Point(sv_left, sv_top+nRoundPix));
		...
		if(nRoundPix > 0) 
		{
			tooltipPath.quadraticCurveTo(	new nexacro.Point(sv_left, sv_top-nRoundPix),
											new nexacro.Point(sv_left-nRoundPix, sv_top));
			tooltipPath.lineTo(left, top);
		}
		else 
		{
		...
		if(nRoundPix > 0) 
		{
			...
			tooltipPath.quadraticCurveTo(	new nexacro.Point(sX + nRoundPix, top),
											new nexacro.Point(fst_left, fst_top+tooltipHeight-nRoundPix));
			tooltipPath.lineTo(fst_left , fst_top + nRoundPix);
			tooltipPath.quadraticCurveTo(	new nexacro.Point(fst_left, fst_top + nRoundPix),
											new nexacro.Point(fst_left + nRoundPix, fst_top));
		}
		else 
		{
			tooltipPath.lineTo(sX, top);
...
if (!nexacro._getChartDisplaytText)
{
    ...
    nexacro._getChartAttrValue = function (attr, rowidx, mask, chart, series)
    {
        ...
        else
        {
            ...
            if (attr._bindtype == 1)
            {
				if(chart._type_name == "FloatChart")	// waterfall
				{
	...
	// RP 86187 :  dddd 는 제외 - _pCellInfo._getDisplayText_date
	nexacro._getChartDisplayFormatText_date = function (date, mask) {
		...
		if(!date || date == "" || !mask || mask == "") return v;
		var dateStr;
		var format;
...
if (!nexacro._BTree)
{
    ...
    _pBTree.add = function (value)
    {
        ...
        else
        {
            ...
            {
                // nexacro._ChartGraphicsLib.Logger.error("Duplicates are not allowed.");
            }
            refRoot = { root: this.root };
