...
if (!nexacro.BasicChart)
{
	...
	_pBasicChart.stacktotalvisible = false;	// 8.29
	_pBasicChart.stacktotallinestyle = null;	// 8.29
	_pBasicChart.stacktotalshadowstyle = null;	// 8.29
	_pBasicChart.stacktotaltextcolor = null;	// 8.29
	_pBasicChart._stacktotallinestyle = null;	// 8.29
	_pBasicChart._stacktotalshadowstyle = null;	// 8.29
	_pBasicChart._stacktotaltextcolor = null;	// 8.29
    ...
    _pBasicChart.on_destroy_contents = function ()
    {
		...
		this.stacktotalvisible = null;	// 8.29
		this.stacktotallinestyle = null;	// 8.29
		this.stacktotalshadowstyle = null;	// 8.29
		this.stacktotaltextcolor = null;	// 8.29		
		this._stacktotallinestyle = null;	// 8.29
		this._stacktotalshadowstyle = null;	// 8.29
		this._stacktotaltextcolor = null;	// 8.29		
    ...
    _pBasicChart.set_stacktotallinestyle = function (val)
    {
        ...
        if (val)
        {
            if (this._stacktotallinestyle == null || this._stacktotallinestyle.value != val)
            {
                ...
                {
                    oldValue = this._stacktotallinestyle.value;
                }
				*/
		...
		if (val) {
			if (this._stacktotalshadowstyle == null || this._stacktotalshadowstyle.value != val) {
				...
				if (this._stacktotalshadowstyle) {
					oldValue = this._stacktotalshadowstyle.value;
				}
				*/
    ...
    _pBasicChart.set_stacktotaltextcolor = function (val)
    {
        ...
        if (val)
        {
            if (this._stacktotaltextcolor == null || this._stacktotaltextcolor.value != val)
            {
                ...
                {
                    oldValue = this._stacktotaltextcolor.value;
                }
				*/
	...
	_pBasicChart._makeVisibleStackGroupingArray = function ()
	{
		...
		var seriesset = this._visibleSeriesset;
		//var seriesset = this.seriesset; //test
		...
		for (i = 0; i < seriesset.length; i++)
		{
			...
			if(s)
			{
				...
				/*if(i%2 == 0)
				{
					s.stackbargroup = "male";
				}
				else
				{
					s.stackbargroup = "fmale";
				}*/
				s._stacktype = s._stacktype == "none" ? this.stacktype : s._stacktype;
				if(s._stacktype == "none")
				{
					...
					else
					{
						...
						orderindex++;
						//none 일 경우 percent 계산 없음 
						//this._stackGroupingArray[nonstackid]._stackPercentBases = {};
						//this._stackGroupingArray[nonstackid]._stackPercentSums = {};
		...
		//trace("stackgrouping_length~~",this._stackGroupingObjList.length);
	};
        ...
        var rotateaxis = this.rotateaxis,
            ...
            function calculCenterBarShift()
            {
                var width = 0,
                    ...
                    groupbarwidth = 0,
                    series;
                // visibleseriesset의 중심 series를 얻기 위해서는 ceil(반올림)이 아니라 floor로 버림 처리되어야 한다.
...
if (!nexacro.ChartBasicSeriesControl)
{
	...
	_pChartBasicSeriesControl.shadowstyle = null;	// 8.20-1 Shadow
	_pChartBasicSeriesControl.barradius = 0;	// 8.20-2 Bar Round
   ...
   _pChartBasicSeriesControl.set_barradius = function (val)
    ...
    _pChartBasicSeriesControl._drawSeriesBars = function ()
    {
        var barlinestyle = this.barlinestyle || "1px solid " + this._color,
			barfillstyle = this._barfillstyle || this._color,
		...
		if(this._chart.stacktotalvisible == true && stacktype == "normal") isTagetStackTotal = true;
		var stackbargroup = this.stackbargroup;	// 8.29
		if(!stackbargroup) stackbargroup = "_default_";
		var objPosition;
		if(!this._chart._stackbargrouplist) this._chart._stackbargrouplist = [];
		var stackbarlist = this._chart._stackbargrouplist;
		if(!stackbarlist[stackbargroup]) stackbarlist[stackbargroup] = [];
		...
		if(barfillstyle instanceof Array) {
			...
			if(barfillstyle && this.autogradation != "none")
			{
				...
				if(bTarget)
				{
					...
					{
						//var lightcolor0 = this._shadeColor(barfillstyle, -30);
						//var lightcolor1 = this._shadeColor(barfillstyle, -20);
						//var lightcolor2 = this._shadeColor(barfillstyle, -10);
        ...
        if (rect && !item)
        {
            ...
            if (itemtextvisible)
            {
				...
				if(!isEffect)
				{
					...
					{
						if(!chart._groupItemTextList) chart._groupItemTextList = [];
						var list = chart._groupItemTextList[this._configIndex];
						if(!list) list = chart._groupItemTextList[this._configIndex] = [];
						var pushdata = {	"left" : left, "right" : right, "bottom" : bottom, 
											"top" : top, "width" : width, "height" : height, "rect" : rect };
						list.push(pushdata);
					}
					else
					{
    ...
    _pChartBasicSeriesControl._drawStackBar = function (stackgroupname, index, left, top, width, height, right, bottom, value)
    {
	   ...
	   if (!seriesGroup) return;
		...
		if(this._chart._stacktotalshadowstyle) rect.set_shadow(this._chart._stacktotalshadowstyle);
		if(this._chart._stacktotallinestyle) rect.set_strokepen(this._chart._stacktotallinestyle || "1px solid #dddddd");
        rect._index = index;
		rect._value = value;
    ...
    _pChartBasicSeriesControl._drawLine = function (datapoints, xoffset, yoffset, axisx, axisy, linestyle, lineopacity, Ishighlight)
    {
        ...
        for (var i = ps; i < points.length; i += ps)
        {
            ...
            if (x1 != prevx || y1 != prevy)
            {
                ...
                // //debug	
                // var circle1 = new nexacro.ChartGraphicsRect(startPointX,  axisy.p2c(y1) + yoffset, 10, 10);
            ...
            // //debug	
            // var circle1 = new nexacro.ChartGraphicsRect(endPointX,  axisy.p2c(y2) + yoffset, 10, 10);
    ...
    _pChartBasicSeriesControl._drawArea = function (datapoints, axisx, axisy, lineareafillstyle, lineareaopacity)
    {
        var points = datapoints.points.slice(0),
			...
			area,
            tickstartgap,
			//plength = datapoints.pointsize.length,
			seriesGroup = this._chart._seriesGroup,
			bCreate = false,
			effect = this._chart_aniframe_obj,
            rotateaxis = this._chart.rotateaxis;
		...
		*/
		...
		//area.set_strokepen("2px solid blue");
        ...
        while (true)
        {
           ...
           // var stacktype = this._stacktype == "none" ? this._chart.stacktype : this._stacktype;
            ...
            if (!areaOpen)
            {
                if (!rotateaxis)
                {
                    area.moveTo(x1Point, /*axisy.p2c(bottom) + shiftPointY + axisy._tickendspace*/y1Point);
					
                }
                else
                {
					area.moveTo(/*axisx.p2c(bottom) +  shiftPointX*/x1Point , y1Point);
			...
			if (/*y1 && y2 && */stepend) // Line path가 그려지고 난 다음 마지막 선 그리기 
			{
				...
				if (!rotateaxis)
                {
					...
					/*var circle1 = new nexacro.ChartGraphicsRect(startX1, startY1 + axisy._tickendspace, 10, 10);
                    circle1.set_strokepen("1px solid black");
                    circle1.set_fillstyle("black");
					circle1.set_id("1" + this._configIndex);
                    this._chart._seriesGroup.addChild(circle1);*/
                    area.lineTo(startX2, startY2 + axisy._tickendspace);
					/*var circle2 = new nexacro.ChartGraphicsRect(startX2, startY2 + axisy._tickendspace, 10, 10);
                    circle2.set_strokepen("1px solid black");
                    circle2.set_fillstyle("red");
					circle2.set_id("2" + this._configIndex);
                    this._chart._seriesGroup.addChild(circle2);*/
    ...
    _pChartBasicSeriesControl._drawCurveLine = function (points, linestyle, lineopacity, Ishighlight,skipmoves)
    {
        ...
        for (var i = 0; i < plength; i++)
        {
            ...
            else if (type == "quadraticCurveTo")
            {
				...
				/*
				 var circle1 = new nexacro.ChartGraphicsRect(x1 ,y1, 10, 10);
				 circle1.set_strokepen("1px solid orange");
				 circle1.set_fillstyle("orange");
				 circle1.set_id("1" + i);
				 this._chart._seriesGroup.addChild(circle1);

				 var circle2 = new nexacro.ChartGraphicsRect(cx ,cy, 10, 10);
				 circle2.set_strokepen("1px solid yellow");
				 circle2.set_fillstyle("yellow");
				 circle2.set_id("2" + i);
				 this._chart._seriesGroup.addChild(circle2);*/
            ...
            else if (type != "quadraticCurveTo" && i != (plength - 2))
            {
				...
				/* var circle1 = new nexacro.ChartGraphicsRect(x1 ,y1, 10, 10);
				 circle1.set_strokepen("1px solid blue");
				 circle1.set_fillstyle("blue");
				 circle1.set_id("1" + i);
				 this._chart._seriesGroup.addChild(circle1);

				 var circle2 = new nexacro.ChartGraphicsRect(cx ,cy, 10, 10);
				 circle2.set_strokepen("1px solid red");
				 circle2.set_fillstyle("red");
				 circle2.set_id("2" + i);
				 this._chart._seriesGroup.addChild(circle2);

			     var circle3 = new nexacro.ChartGraphicsRect(x2 ,y2, 10, 10);
				 circle3.set_strokepen("1px solid green");
				 circle3.set_fillstyle("green");
				 circle3.set_id("3" + i);
				 this._chart._seriesGroup.addChild(circle3);
    ...
    _pChartBasicSeriesControl._drawBarItemText = function (left, right, bottom, top, width, height, item, stackgroupname, index, uservalue)
    {
		...
		{
			//var itemtexttype = this.itemtexttype,
			var itemtextmask = this.itemtextmask,
			itemtextfont = this.itemtextfont,
			itemtextcolor = this._chart._stacktotaltextcolor,
        ...
        if (!nexacro._isNull(itemText))
        {
            ...
            // //debug	
            // var circle1 = new nexacro.ChartGraphicsEllipse(textLeft, textTop, 6, 6);
            // circle1.set_strokepen("1px solid black");
            // circle1.set_fillstyle("red");
