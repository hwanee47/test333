...
if (!nexacro.Frame) {
	...
	_pChildFrame.destroyComponent = function () {
		if (this._window_type == 1 || this._window_type == 4) {
			...
			if (this.opener && this.opener.parent) {
				this.opener.parent._setModalUnlock();
				this.opener.parent._runCallback();
			}
		}
				...
				if (_window && (_window._wheelZoom != undefined) && (_window._wheelZoom != 100)) {
					if (this.form._is_loaded) {
						...
						if (frameElem) {
							...
							frameElem.setElementPosition(left * _window._wheelZoom / 100.0, top * _window._wheelZoom / 100.0);
	...
	_pChildFrame._showModalWindow = function (str_id, _parent_frame, arr_arg, opener, _lockmode) {
		...
		parent_frame._setModalLock(this._overlaycolor);
		...
		if (this._window_type == 1) {
			if (this._isNotProcessedWheelZoom == true) {
				...
				if (wheelZoomScale != 1.0) {
					...
					if (frameElem) {
						frameElem.setElementZoom(_window._wheelZoom);
						frameElem.setElementPosition(this.left, this.top);
						frameElem.setElementSize(recalculated_pos.width, recalculated_pos.height, true);
					}
				}
			}
		}
